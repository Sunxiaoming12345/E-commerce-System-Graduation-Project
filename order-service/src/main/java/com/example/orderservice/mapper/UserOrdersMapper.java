...
public interface UserOrdersMapper {
  /*  //预下单
   ...
   @Insert("insert into orders(user_id, order_number, order_status, payment_method, total_amount, shipping_address, receiver_name, receiver_phone, create_time) values(#{userId},#{orderNumber},#{orderStatus},#{paymentMethod},#{totalAmount},#{shippingAddress},#{receiverName},#{receiverPhone},#{createTime}) ")
    ...
    // 获取用户最近一次订单的收货信息
    Orders selectLastReceiverInfo(@Param("userId") Long userId);

    // 更新收货信息
    void updateReceiver(@Param("orderId") Long orderId, @Param("receiverName") String receiverName,
                        @Param("receiverPhone") String receiverPhone, @Param("shippingAddress") String shippingAddress);
    ...
    java.util.List<com.example.mailadmin.entity.OrderItems> selectOrderItemsByOrderIds(@Param("orderIds") java.util.List<Long> orderIds);
